...
const checkIfMovieAccessible = async (req, res) => {
  ...
  try {
    ...
    let isMovieAccessible = purchasedMovies.some(
...
const addMovie = async (req, res) => {
  ...
  try {
    const user = await Users.findById(user_id);
    const isMovieInList = user.purchasedMovies.filter(
      (movie) => movie.movieId === movieId
    );
...
module.exports.checkIfMovieAccessible = checkIfMovieAccessible;
