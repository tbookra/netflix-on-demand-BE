...
const register = async (req, res) => {
  ...
  try {
    ...
    const emailExist = await Users.findOne({
      $and: [{ email }, { status: "active" }, { isConfirmed: true }],
    });
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    if (!user)
      return res.json({ error: "This Email hasn't been confirmed yet!" });
    ...
    if (req.session.changePassword) {
      ...
      res
        ...
        .json({
          token,
          userObj: { email: user.email, full_name: user.full_name },
        });
...
const newPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user)
      return res.json({
        error: "No such user exists. would you like to register?",
      });
    let comperdPassword = await bcrypt.checkPassword(
      new_password,
      user.password
    );
    if (comperdPassword)
      return res.json({ error: "You have to choose a new password!" });
    ...
    await Users.updateOne(
      { _id: user._id },
      { $set: { password: newHashPassword, passwordLastModified: Date.now() } }
    );
    const userObj = {
...
const confirmed = async (req, res, next) => {
  ...
  try {
    ...
    await Users.updateOne(
      { _id: newUser._id },
      { $set: { status: "active", isConfirmed: true } }
    );
    ...
    res
      ...
      .json({
        token,
        userObj: { email: newUser.email, full_name: newUser.full_name },
      });
  } catch (e) {
    console.log(e);
  }
